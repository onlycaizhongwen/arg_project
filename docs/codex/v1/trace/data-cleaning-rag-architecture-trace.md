...
| 真实 Cross-Encoder 精排 | 已有兼容接口和降级，尚未接入真实重排模型 | 阶段 2 可接受 |
| 完整权限模型 | 已有最小标签交集过滤，尚未接入真实用户、角色、组织、资源授权 | 阶段 2 可接受，生产前需增强 |
| 任务阶段耗时 | 当前有状态与错误原因，阶段级耗时/日志不足 | MVP 收口项 |
| 人工重试入口 | Worker 内部可重试，但 API 未提供人工重试接口 | MVP 可暂缓 |
| 删除/更新/索引重建 | 未实现文档删除、版本更新、向量删除和索引重建 | 阶段 2/3 |
...
建议下一步补阶段 2 一致性 trace，然后进入阶段 3，优先做文档更新/删除/索引重建、真实重排模型联调和权限治理增强；如需要线上验收，则先注入真实 `DASHSCOPE_API_KEY` 验证通义 `text-embedding-v4`。
